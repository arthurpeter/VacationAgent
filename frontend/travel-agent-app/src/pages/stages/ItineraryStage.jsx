  ...
  const refreshGlobalState = useCallback(async () => {
    try {
      ...
      if (res && res.ok) {
        ...
        setDiscoveryResults(data.resolved_attractions || []);
        setSearchLocation(data.search_location || data.data?.destination || "");
    ...
    <PageTransition className="w-full h-full overflow-y-auto bg-gray-50 p-4 md:p-8 relative">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-6 h-[calc(100vh-120px)]">
        <div className="bg-white px-5 py-3 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between">
          ...
          <button className="bg-gray-900 text-white px-8 py-3 rounded-2xl font-black text-sm flex items-center gap-2 hover:bg-blue-600 transition-all shadow-lg shadow-gray-200">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              ...
              {!discoveryResults.length && !isSearching && (
                  <div className="h-full flex flex-col items-center justify-center opacity-20 py-20">
                      ...
                      <p className="font-bold text-lg text-center">Your discovery feed is empty.<br/><span className="text-sm font-normal">Use the search above to unlock the magic of {searchLocation.split(',')[0]}.</span></p>
                  </div>
              )}
            </div>
          </div>
