...
if not os.path.exists("results.csv"):
    print(
        "results.csv nu a fost gasit. Folosind datele de test hardcodate pentru generarea ploturilor."
    )
    import io
...
df["Concurrency_Label"] = df.apply(
    lambda r: f"{r['Concurrency']} (P={r['Pipelining']})", axis=1
)
...
line1 = ax1.plot(
    df_auth["Concurrency_Label"],
    df_auth["Requests_Per_Second"],
    marker="o",
    color=color,
    linewidth=2.5,
...
df_offers["Cache_Status"] = df_offers["Cache"].apply(
    lambda x: "With Cache" if str(x).lower() == "yes" else "WithoutCache"
)
...
ax1.set_ylabel("Throughput (RPS)", color=color)
line1 = ax1.plot(
    df_sse["Concurrency"].astype(str),
    df_sse["Requests_Per_Second"],
    marker="^",
    color=color,
    linewidth=2.5,
    label="Throughput (RPS)",
...
ax1.set_ylabel("Throughput (RPS)", color=color)
line1 = ax1.plot(
    df_or["Concurrency"].astype(str),
    df_or["Requests_Per_Second"],
    marker="o",
    color=color,
    linewidth=2.5,
    label="Throughput (RPS)",
