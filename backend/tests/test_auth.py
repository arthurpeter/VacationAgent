...
def test_refresh_token_rotation(mock_blacklist, db_session):
    ...
    # Injectam un TokenPayload fals direct in dependenta rutei pentru a sari peste validarea CSRF/Cookie
...
def test_vacation_session_isolation(db_session):
    user_attacker = User(email="attacker@example.com", hashed_password="pwd", is_verified=True)
    user_victim = User(email="victim@example.com", hashed_password="pwd", is_verified=True)
    ...
    with TestClient(app) as client:
        ...
        patch_response = client.patch(f"/session/{session_victim.id}/details", json=patch_payload, headers=headers)
