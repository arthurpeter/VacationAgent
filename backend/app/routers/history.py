...
async def get_all_history(
    ...
    stmt = (
        ...
        .where(
            models.Vacation.user_id == token.sub, models.Vacation.is_finalized == True
        )
        .order_by(models.Vacation.created_at.desc())
        .options(
...
async def get_vacation_details(
    ...
    stmt = select(models.Vacation).where(
        models.Vacation.id == vacation_id, models.Vacation.user_id == token.sub
    )
...
async def delete_history(
    ...
    if not vacation:
        raise HTTPException(
            status_code=404, detail="Vacation history not found or unauthorized"
        )
