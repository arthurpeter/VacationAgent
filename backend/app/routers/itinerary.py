...
async def update_search_location(
    ...
    if not session:
        log.warning(
            f"Unauthorized access attempt to session {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
...
async def get_itinerary_state(
    ...
    if not session:
        log.warning(
            f"Unauthorized access attempt to session {session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
...
async def update_mobility(
    ...
    await graph.aupdate_state(
        config, {"mobility_config": validated_config.model_dump(mode="json")}
    )
...
async def trigger_public_transport_search(
    ...
    if not session:
        log.warning(
            f"Unauthorized logistics access: {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
...
async def trigger_pace_change(
    ...
    try:
        await graph.aupdate_state(config, {"pace": data.pace})
...
async def trigger_schedule_action(
    ...
    if not session:
        log.warning(
            f"Unauthorized schedule access: {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
...
async def update_trip_details(
    ...
    if not session:
        log.warning(
            f"Unauthorized trip details access: {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
        await graph.aupdate_state(config, {"trip_details": data.trip_details})
...
async def update_custom_timeline(
    ...
    if not session:
        log.warning(
            f"Unauthorized custom timeline access: {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
        await graph.aupdate_state(config, {"user_timeline": data.user_timeline})
...
async def set_transit_mode(
    ...
    if not session:
        log.warning(
            f"Unauthorized transit mode access: {data.session_id} by user {token.sub}"
        )
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
                ...
                for event in day.get("events", []):
                    if event.get("id") == data.leg_key:
                        ...
                        if data.mode in alternatives:
                            ...
                            leg["polyline"] = alternatives[data.mode].get(
                                "polyline", ""
                            )
                            leg["distance_text"] = alternatives[data.mode].get(
                                "distance_text", {}
                            )
                            updated = True
                        break
        ...
        await graph.aupdate_state(
            config, {"schedule": schedule, "user_timeline": user_timeline}
        )
