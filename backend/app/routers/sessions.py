...
from app.services.agents.itinerary_graph import (
    generate_graph as generate_itinerary_graph,
)
...
async def update_session_details(
    ...
    stmt = (
        select(models.VacationSession)
        .options(selectinload(models.VacationSession.companions))
        .filter_by(id=session_id, user_id=access_token.sub)
    )
...
async def create_vacation_session(
    ...
    new_session = models.VacationSession(user_id=access_token.sub)
    db.add(new_session)
    await db.commit()
...
async def get_sessions(
    ...
    stmt = select(models.VacationSession).filter_by(user_id=access_token.sub)
    result = await db.execute(stmt)
...
async def compile_session(
    ...
    if not session:
        raise HTTPException(
            status_code=404, detail="Active vacation session not found for user"
        )
    ...
    try:
        ...
        if current_state and current_state.values:
            ...
            mobility_config = current_state.values.get("mobility_config", {}).get(
                "strategies", {}
            )
    except Exception as graph_err:
        log.warning(
            f"LangGraph state not initialized or missing for session {session_id}: {str(graph_err)}"
        )
    ...
    attraction_map = {}
    if attraction_ids:
        attr_stmt = select(models.GlobalAttraction).where(
            models.GlobalAttraction.id.in_(list(attraction_ids))
        )
    ...
    for day in raw_schedule:
        ...
        for event in day.get("events", []):
            ...
            if (
                ...
                and event_id in attraction_map
            ):
            ...
            if transit_leg and transit_leg.get("is_verified"):
                ...
                mode_data = (
                    alternatives.get(active_mode) if alternatives else transit_leg
                )

                if mode_data:
                    base_event["transit_path"] = {
                        "mode": active_mode,
                        "duration_mins": mode_data.get(
    ...
    vacation_stmt = select(models.Vacation).where(
        models.Vacation.session_id == session.id
    )
    ...
    try:
        ...
        await db.rollback()
        log.error(
            f"Database transaction failure during compilation commit: {str(commit_err)}"
        )
        raise HTTPException(
            status_code=500, detail="Database write error during validation pass"
        )
...
async def process_email_and_notify(user_id: str, email_to: str, vacation_payload: dict):
    ...
    try:
        ...
        log.error(
            f"Background serialization or mailing runner failure: {str(e)}",
            exc_info=True,
        )
        async for db in get_db():
            db.add(
...
async def finalize_and_email_session(
    ...
    """
    vacation_stmt = select(models.Vacation).where(
        models.Vacation.session_id == session_id, models.Vacation.user_id == token.sub
    )
    ...
    if not user or not user.email:
        raise HTTPException(
            status_code=400,
            detail="Authenticated profile lacks a valid communication routing address.",
        )

    session_stmt = select(models.VacationSession).where(
        models.VacationSession.id == session_id
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Database state transaction failure: {str(commit_err)}",
        )
...
async def get_vacation_session(
    ...
    if not session:
        log.warning(
            f"Vacation session {session_id} not found for user: {access_token.sub}"
        )
