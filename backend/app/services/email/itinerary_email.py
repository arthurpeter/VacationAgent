...
async def send_vacation_blueprint_email(to_email: str, session_data: dict) -> bool:
    ...
    if not settings.SMTP_USER or not settings.SMTP_PASSWORD:
        log.error("SMTP credentials missing from application environment configurations.")
        return False

    normalized_data = dict(session_data)
    if "accommodation_price" in normalized_data and "accommodation_price" not in normalized_data:
        normalized_data["accommodation_price"] = normalized_data.get("accommodation_price")
        normalized_data["accommodation_ccy"] = normalized_data.get("accommodation_ccy")
        normalized_data["accommodation_name"] = normalized_data.get("accommodation_name")
        normalized_data["accommodation_address"] = normalized_data.get("accommodation_address")
    ...
    try:
        log.info(f"🖨️ Compiling vector PDF blueprint for session context matching...")
        pdf_bytes = generate_itinerary_pdf(normalized_data)
        
        safe_filename = f"TuRAG_Itinerary_{destination.replace(',', '').replace(' ', '_')}.pdf"
    ...
    try:
        log.info(f"🚀 Initializing secure SMTP socket link to {settings.SMTP_HOST}:{settings.SMTP_PORT}...")
        with smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT) as server:
            server.starttls()
            server.login(settings.SMTP_USER, settings.SMTP_PASSWORD)
            server.send_message(msg)
            
        log.info(f"🎉 Success! Vacation blueprint email with PDF attachment dispatched to {to_email}")
        return True
    except Exception as e:
        log.error(f"Failed to dispatch vacation blueprint email via SMTP: {e}", exc_info=True)
        return False