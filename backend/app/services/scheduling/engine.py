...
class ScheduleEngine:
    ...
    # -------------------------------------------------------------------------
    # Internal helpers for the two-pass generate_schedule
    # -------------------------------------------------------------------------
    ...
    def _build_day_context(
        ...
        if day_idx == 0:
            ...
            # start clock after hotel check-in, but not before ready
            start_clk_dt = max(
            ...
            wakeup_dt = max(ideal_wakeup - timedelta(minutes=shift_mins), earliest_allowed)
            ready_dt = wakeup_dt + timedelta(minutes=90)
            start_clk_dt = ready_dt
    ...
    def _estimate_day_free_mins(
        ...
        # ------------------------------------------------------------------ #
        # Primary path: read actual solver output                            #
        # ------------------------------------------------------------------ #
        if solved_route and len(solved_route) >= 2:
            # Find the last non-logistics, non-meal departure minute and location.
            # The end node (index -1) is always the logistics anchor; we want
            # the departure minute of the last real attraction node.
            last_dep_min = ctx['start_min']  # fallback if no real nodes found
            last_real_lat, last_real_lon = self.hotel_coords

            for step in solved_route[1:-1]:  # skip start and end anchors
                n = step['node']
                if n['type'] in ('attraction',) and n['id'] not in ('lunch',):
                    ...
                    last_dep_min = step['dep_min']
                    # meal is placed near previous real location — don't shift coords

            # Time remaining after the last committed departure, before cutoff
            slack = end_min - last_dep_min

            # Subtract: transit from last real location → end node (hotel/airport)
            est_return = self._get_base_transit_mins(
                last_real_lat, last_real_lon,
                ctx['end_node_coords'][0], ctx['end_node_coords'][1],
            )
            ret_mins, _ = self._resolve_transit_leg(
                last_real_lat, last_real_lon,
                ctx['end_node_coords'][0], ctx['end_node_coords'][1],
                transit_lookup, est_return,
            )
            slack -= ret_mins

            # Subtract: the candidate POI itself (duration + transit to it)
            if candidate_poi is not None:
                ...
                est_to_poi = self._get_base_transit_mins(
                    last_real_lat, last_real_lon,
                    candidate_poi['latitude'], candidate_poi['longitude'],
                )
                transit_to_poi, _ = self._resolve_transit_leg(
                    last_real_lat, last_real_lon,
                    candidate_poi['latitude'], candidate_poi['longitude'],
                    transit_lookup, est_to_poi,
                )
        ...
        # ------------------------------------------------------------------ #
        # Fallback path: arithmetic estimate (no solved route yet)           #
        # ------------------------------------------------------------------ #
        ...
        for p in assigned_pois:
            est = self._get_base_transit_mins(prev_lat, prev_lon, p['latitude'], p['longitude'])
            t, _ = self._resolve_transit_leg(prev_lat, prev_lon, p['latitude'], p['longitude'], transit_lookup, est)
            transit_used += t
            prev_lat, prev_lon = p['latitude'], p['longitude']

        # return leg from last POI to end node
    ...
    def _build_route_events(
        ...
        # ---- hotel departure block (all days except day 0) ------------------
        if day_idx != 0:
            leave_hotel_dt = base_dt + timedelta(minutes=route[0]['dep_min'])
            day_plan.append({
                "type": "attraction", "id": f"start_hotel_{day_idx}",
                "name": "Start Day at Hotel", "bucket": "logistics",
        ...
        for i in range(1, len(route) - 1):
            ...
            if c_node['type'] == 'meal':
                ...
                t_mins, t_leg = self._resolve_transit_leg(
                    last_real_lat, last_real_lon, c_node['lat'], c_node['lon'], transit_lookup, est_mins
                )
            ...
            if c_node['type'] == 'meal':
                ...
                day_plan.append({
                    "type": "attraction", "id": p_data['id'], "name": p_data['name'],
                    "bucket": c_node['bucket'],
                    "start_time": node_start.strftime("%H:%M"),
                    "end_time": node_end.strftime("%H:%M"),
                    "transit_mins": t_mins, "unknown_hours_warning": False,
                    "latitude": p_data['latitude'], "longitude": p_data['longitude'],
                    "image_url": p_data.get('image_url'),
                    "transit_leg": t_leg,
                })

        # ---- return / departure bookend -------------------------------------
        end_r = route[-1]
        ...
        if day_idx == total_days - 1:
            ...
            day_plan.append({
                "type": "attraction", "id": "dep_airport",
                "name": "Airport Check-in & Security", "bucket": "logistics",
            ...
            day_plan.append({
                "type": "attraction", "id": f"return_hotel_{day_idx}",
                "name": "Return to Hotel", "bucket": "logistics",
                "start_time": final_arr.strftime("%H:%M"),
    ...
    def _build_node_list(
        ...
        nodes: List[Dict] = []
        nodes.append({
            "id": "start", "type": "logistics",
            "lat": self.hotel_coords[0], "lon": self.hotel_coords[1],
            "duration": 0, "penalty": 0,
        })
        ...
        for p in daily_pool:
            ...
            nodes.append({
                "id": p['id'], "type": "attraction",
                "lat": p['latitude'], "lon": p['longitude'],
                "duration": raw_dur, "bucket": bucket, "penalty": penalty,
                "open_min": windows[0], "max_start_min": windows[1],
                "poi_data": p,
            })

        if start_min < 15 * 60 and end_min > 13 * 60:
            nodes.append({
                "id": "lunch", "type": "meal", "name": "Lunch Break",
                "lat": self.hotel_coords[0], "lon": self.hotel_coords[1],
        ...
        nodes.append({
            "id": "end", "type": "logistics",
            "lat": end_node_coords[0], "lon": end_node_coords[1],
            "duration": 0, "penalty": 0,
        })
    ...
    # -------------------------------------------------------------------------
    # generate_schedule — two-pass implementation
    # -------------------------------------------------------------------------
    ...
    def generate_schedule(
        ...
        # ------------------------------------------------------------------ #
        # Pre-processing: split home vs excursion, build clusters            #
        # ------------------------------------------------------------------ #
        home_pois: List[Dict] = []
        ...
        # Group excursion POIs by proximity and map each group to a full day
        ...
        # day_idx → cluster index mapping (home days only)
        day_to_cluster: Dict[int, int] = {
            day_idx: i for i, day_idx in enumerate(home_days)
        }

        # Cluster centroid lookup (used in Pass 2 for proximity scoring)
        ...
        # ------------------------------------------------------------------ #
        # Pass 1 — solve each day strictly within its assigned cluster        #
        # ------------------------------------------------------------------ #

        # Structures we carry across passes
        assigned_ids: set = set()
        # day_idx → list of POI dicts that were actually solved into the route
        day_assigned_pois: Dict[int, List[Dict]] = {d: [] for d in active_days}
        # day_idx → solved route (raw OR-Tools output)
        day_routes: Dict[int, List[Dict]] = {}
        # day_idx → day context
        day_contexts: Dict[int, Dict] = {}
        # day_idx → node list used in last solve (needed for re-solve in Pass 2)
        day_node_lists: Dict[int, List[Dict]] = {}

        schedule_buffer: Dict[int, Dict] = {}  # day_idx → partial schedule entry

        for day_idx in active_days:
            ctx = self._build_day_context(day_idx, total_days, excursion_day_map, profile)
            ...
            # ---- arrival-day logistics bookend ------------------------------
            if day_idx == 0:
                bags_claim_end = self.arrival_dt + timedelta(minutes=self.airport_egress_mins)
                day_plan.append({
                    "type": "attraction", "id": "arr_airport",
                    "name": "Customs & Baggage Claim", "bucket": "logistics",
                ...
                day_plan.append({
                    "type": "attraction", "id": "arr_hotel",
                    "name": "Check-in & Settle at Hotel", "bucket": "logistics",
            ...
            # ---- select this day's POI pool (cluster-strict) ----------------
            if ctx['is_excursion']:
                daily_pool = [
                    p for p in excursion_day_map[day_idx]
                    if p['id'] not in assigned_ids
                ]
            elif day_idx in home_days:
                cluster_idx = day_to_cluster[day_idx]
                daily_pool = [
                    p for p in balanced_clusters.get(cluster_idx, [])
                    if p['id'] not in assigned_ids
                ]
            else:
                # arrival or departure day — no sightseeing pool from clusters
                daily_pool = []

            # ---- build nodes and solve --------------------------------------
            nodes = self._build_node_list(daily_pool, ctx, ctx['end_node_coords'])
            day_node_lists[day_idx] = nodes
            route = self._solve_day_route(nodes, ctx['start_min'], ctx['end_min'], transit_lookup)
            day_routes[day_idx] = route

            # ---- collect which POIs the solver actually kept ----------------
            ...
            schedule_buffer[day_idx] = {
                ...
                "events": day_plan,
                # mutable — Pass 2 may rebuild this
                "_day_plan_prefix": day_plan,
            }

        # ------------------------------------------------------------------ #
        # Pass 2 — absorb leftover POIs into days with remaining capacity     #
        # ------------------------------------------------------------------ #

        # Build the leftover list: everything not yet assigned
        priority_rank = {"must": 0, "want": 1, "optional": 2}
        leftovers: List[Dict] = [p for p in pois if p['id'] not in assigned_ids]
        # Sort by priority first (must → want → optional), then by id for stability
        leftovers.sort(key=lambda p: (
            priority_rank.get(p.get('bucket', 'want').lower(), 1),
            p['id'],
        ))

        # Candidate days for absorption: home days + day-0 + last day,
        # NOT excursion days (their pool is geographically fixed).
        candidate_days = [
            d for d in active_days if d not in excursion_day_map
        ]

        for leftover in leftovers:
            if leftover['id'] in assigned_ids:
                continue  # already placed by a previous iteration

            # Score each candidate day: prefer days with (a) most free time
            # and (b) centroid closest to this POI.
            best_day = None
            best_score = None  # lower is better: (–free_mins, proximity_km)

            for day_idx in candidate_days:
                ctx = day_contexts[day_idx]
                # Pass the candidate POI and the actual solved route so the
                # estimator uses real solver output rather than arithmetic guesses.
                ...
                # free_mins already has the candidate's own cost subtracted —
                # > 0 means the day can absorb it with slack to spare.
                if free_mins <= 0:
                    continue
                ...
                # Primary key: most free time (negated so lower = better).
                # Secondary key: proximity to cluster centroid.
            ...
            # Tentatively add this POI to best_day's pool and re-solve
            ...
            # Check that the solver actually included the leftover POI
            solved_in_trial = set()
            ...
            # Commit: update all tracking structures for best_day
            log.debug(
            ...
            # Rebuild the day's event list from the new route
            old_entry = schedule_buffer[best_day]
            current_date = ctx['current_date']
            base_dt = datetime.combine(current_date, time.min)
            wakeup_dt = ctx['wakeup_dt']

            # Reconstruct logistics prefix (same logic as Pass 1)
            new_day_plan: List[Dict] = []

            if best_day == 0:
                # Re-emit arrival logistics (unchanged)
                bags_claim_end = self.arrival_dt + timedelta(minutes=self.airport_egress_mins)
                new_day_plan.append({
                    "type": "attraction", "id": "arr_airport",
                    "name": "Customs & Baggage Claim", "bucket": "logistics",
        ...
        for p in pois:
            if p['id'] not in assigned_ids:
                ...
                excluded[bucket].append({
                    "id": p["id"], "name": p["name"],
                    "bucket": bucket, "type": "attraction",
                })

        return {"status": "success", "schedule": schedule, "excluded": excluded}

    # -------------------------------------------------------------------------
    # Unchanged methods below — do not modify
    # -------------------------------------------------------------------------
