...
async def information_collector(state: DiscoveryState) -> dict:
    ...
    current_knowledge = (
        "\n".join(
            [
                f"- {k}: {v}"
                for k, v in state.get("extracted_data", {}).items()
                if v is not None
            ]
        )
        if state.get("extracted_data")
        else "No information collected yet."
    )
...
async def db_validator(state: DiscoveryState) -> dict:
    ...
    if new_info:
        ...
        for k, v in new_info.items():
            if (
                is_llm_null(v)
                or k == "is_change_request"
                or k == "passengers_confirmed"
            ):
                continue
        ...
        async with SessionLocal() as db:
            if update_values:
                await db.execute(
                    ...
                    .where(
                        VacationSession.id == session_id,
                        VacationSession.user_id == user_id,
                    )
    ...
    if all(refreshed_data.get(f) for f in mandatory):
        ...
        basic_logic = (
            d1 < d2
            and d1 > datetime.now()
            and refreshed_data["departure"] != refreshed_data["destination"]
            and (refreshed_data["adults"] or 0) >= 1
        )
        ...
        infant_total_count = (refreshed_data.get("infants_in_seat") or 0) + (
            refreshed_data.get("infants_on_lap") or 0
        )
        child_total_count = refreshed_data.get("children") or 0

        consistency_ok = (
            infants_from_ages == infant_total_count
            and children_from_ages == child_total_count
        )
...
async def responder(state: DiscoveryState) -> dict:
    ...
    for msg in older_messages:
        if msg.type == "human":
            ...
            short_content = (
                content_str if len(content_str) < 300 else content_str[:300] + "..."
            )
    ...
    user_history_str = state.get(
        "user_history", "No prior travel history recorded with us."
    )
    ...
    response = await llm_with_tools.ainvoke(
        [SystemMessage(content=system_instructions)] + current_messages
    )
    ...
    if action == "initial_fetch":
        ...
        async with SessionLocal() as db:
            ...
            if len(existing_places) >= 10:
                log.info(
                    f"CACHE HIT: Showing {len(existing_places)} existing places from DB."
                )

                hit_ids = [p.id for p in existing_places]
                await GlobalAttraction.track_search_metrics(db, hit_ids)

                updates["resolved_attractions"] = Overwrite(
                    [format_cached_poi(p) for p in existing_places]
                )
                return updates
            ...
            for name in attraction_names:
                poi_data = await autosuggest_places(
                    name, coords["lat"], coords["lon"], limit=1
                )
                if not poi_data:
                    continue
                ...
                stmt = select(GlobalAttraction).where(
                    GlobalAttraction.external_place_id == xid
                )
        ...
        prompt = custom_search_prompt.format(
            destination=f"{city}, {country}", user_query=user_query
        )
        ...
        async with SessionLocal() as db:
            for name in attraction_names:
                ...
                if cached:
                    ...
                    log.info(
                        f"NEW ATTRACTION: {name} not in DB, fetching OTM details..."
                    )
                    resolved = await get_place_details(xid)
                    if resolved:
                        resolved["city"] = city
...
async def enrich_single_attraction_node(poi_data: dict) -> dict:
    ...
    clean_hours = {
        day: (val if val and val.strip() else "N/A") for day, val in raw_hours.items()
    }
...
async def save_attractions_to_db(state: ItineraryState) -> dict:
    ...
    async with SessionLocal() as db:
        for poi_data in resolved:
            stmt = select(GlobalAttraction).where(
                GlobalAttraction.external_place_id == poi_data.get("external_place_id")
            )
            result = await db.execute(stmt)
            ...
            if not existing_place:
                new_attraction = GlobalAttraction(
                    ...
                    recommended_duration_mins=poi_data.get(
    ...
    return {"resolved_attractions": Overwrite(updated_resolved_list), "action": "idle"}


async def picking_transit(state: ItineraryState) -> dict:
    action = state.get("action")
    location = state.get("data", {}).get("destination") or state.get(
        "search_location", "Unknown Location"
    )
    ...
    if action == "search_public_transport_offers":
        if (
            config_dict.get("strategies", {})
            .get("public_transport", {})
            .get("details_loaded")
        ):
            return {"action": "idle"}
        return await _execute_transit_research(
            from_date, to_date, location, config_dict
        )
        ...
        return await _execute_mobility_research(
            from_date, to_date, location, attraction_locations
        )
...
async def _execute_transit_research(
    from_date: str, to_date: str, location: str, config_dict: dict
) -> dict:
    duration_days = 7
...
async def _execute_rental_research(
    from_date: str, to_date: str, location: str, config_dict: dict
) -> dict:
    duration_days = 7
    month_year = datetime.now().strftime("%B %Y")
...
async def _execute_mobility_research(
    from_date: str, to_date: str, location: str, locations: set
) -> dict:
    ...
    return {"mobility_recommendation": recommendation, "action": "idle"}


async def _execute_pace_research(
    from_date: str, to_date: str, location: str, pois: List[dict]
) -> dict:
    duration_days = 7
    month_year = datetime.now().strftime("%B %Y")
    ...
    return {"pace_recommendation": recommendation, "action": "idle"}
...
async def _generate_schedule(state: ItineraryState) -> dict:
    ...
    try:
        ...
        for state_poi in pois_state:
            ...
            duration = (
                state_poi.get("time_to_spend") or db_poi.recommended_duration_mins
            )
...
async def _recalculate_timeline(state: ItineraryState) -> dict:
    ...
    try:
        ...
        result = await anyio.to_thread.run_sync(
            engine.recalculate_user_timeline,
            user_timeline,
            engine_pois,
            existing_transit_legs,
        )
...
async def _sync_transit(state: ItineraryState) -> dict:
    ...
    try:
        ...
        log.error(
            f"Failed to enrich POIs inside sync transit worker node: {e}", exc_info=True
        )
        return {}
    ...
    for day in current_schedule:
        ...
        for i in range(1, len(geo_events)):
            ...
            if (
                prev_evt.get("id") == "arr_airport"
                or "airport" in prev_evt.get("name", "").lower()
            ):
                origin_name = state.get("data", {}).get("airport_name")

            if (
                curr_evt.get("id") == "dep_airport"
                or "airport" in curr_evt.get("name", "").lower()
            ):
    ...
    arrival_dt = parse_dt(
        trip_details.get("arrival_dt") or trip_details.get("arrival_time")
    )
    departure_dt = parse_dt(
        trip_details.get("departure_dt") or trip_details.get("departure_time")
    )
    ...
    recalc_result = await anyio.to_thread.run_sync(
        engine.recalculate_user_timeline,
        user_days_poi_ids,
        engine_pois,
        existing_transit_legs,
...
if __name__ == "__main__":
    print(
        "This module is not meant to be run directly. It provides the agent nodes and decisional edges."
    )
