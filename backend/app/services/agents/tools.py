    ...
    with ddgs_lock:
        for attempt in range(max_retries):
            try:
                ...
                snippets = [
                    f"URL: {res['href']} | Info: {res['body']}" for res in results
                ]
                ...
                log.info(
                    f"Web search successful. Returning combined snippets: {safe_result[:100]}"
                )
                return safe_result
                ...
                if (
                    ...
                    or "httpx" in error_msg
                ):
                    log.warning(
                        f"DDGS Rate limit/Network error for '{query}'. Attempt {attempt + 1}/{max_retries}. Retrying in {base_delay}s..."
                    )
    ...
    with ddgs_lock:
        for attempt in range(max_retries):
            try:
                ...
                links = {
                    res["href"]: res.get("title", "No title available")
                    for res in results
                    if "href" in res
                }
                ...
                combined_links = "\n".join(
                    [f"{url}: {description}" for url, description in links.items()]
                )
        ...
        log.error(
            f"Link finding failed after {max_retries} attempts due to rate limits."
        )
...
def get_transit_directions(origin: str, destination: str):
    ...
    try:
        directions = gmaps.directions(origin, destination, mode="transit")
        ...
        for step in route["steps"]:
            if step["travel_mode"] == "TRANSIT":
                ...
                line = transit["line"].get(
                    "short_name", transit["line"].get("name", "Transit")
                )
                vehicle = transit["line"]["vehicle"]["name"]
                steps.append(
                    f"Take {vehicle} ({line}) from {transit['departure_stop']['name']} to {transit['arrival_stop']['name']}"
                )
            else:
                clean_text = (
        ...
        print(
            f"Transit directions from {origin} to {destination}: Duration: {duration}, Distance: {distance}, Fare: {fare}, Instructions: {instructions}"
        )
