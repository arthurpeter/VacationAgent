...
class ExtractionResult(BaseModel):
    ...
    departure: Optional[str] = Field(
        None, description="'City, country_code' for departure, e.g., 'New York, US'"
    )
    destination: Optional[str] = Field(
        None, description="'City, country_code' for arrival, e.g., 'Paris, FR'"
    )
    ...
    currency: Optional[str] = Field(
        None, description="ISO currency code, e.g., 'USD', 'EUR'"
    )
    budget: Optional[int] = Field(
        None,
        description="The maximum total budget the user wants to spend for the trip, as an integer (e.g., 5000)",
    )

    adults: Optional[int] = Field(None, description="Number of adults (18+ years old)")
    children: Optional[int] = Field(
        None, description="Number of children (2-17 years old)"
    )
    infants_in_seat: Optional[int] = Field(
        None, description="Number of infants (0-2 years old) with their own seat"
    )
    infants_on_lap: Optional[int] = Field(
        None, description="Number of infants (0-2 years old) sitting on an adult's lap"
    )
...
class TransitEnrichmentSchema(BaseModel):
    official_link: str = Field(
        description="The URL to the official city public transport website or tourist pass page."
    )
    pass_price_est: float = Field(
        description="The estimated cost for a standard 48h or 72h tourist transport pass. Numeric value only."
    )
    currency: str = Field(
        description="The 3-letter ISO currency code for the price (e.g., EUR, RON, USD)."
    )
    ...
    details_found: bool = Field(
        description="Set to true if specific 2026 price data was found, false if estimated."
    )


class RentalEnrichmentSchema(BaseModel):
    official_link: str = Field(
        description="URL to a major rental aggregator or local provider in the city."
    )
    daily_price_est: float = Field(
        description="Estimated daily cost for an economy car hire."
    )
    currency: str = Field(description="ISO currency code (e.g., EUR).")
    ztl_warning: bool = Field(
        description="True if the city has strict Limited Traffic Zones (ZTL) or high congestion fees."
    )
    operating_hours: Dict[str, str] = Field(
...
class RecommendationSchema(BaseModel):
    recommendation: str = Field(
        description="A concise, actionable recommendation for the user based on the itinerary context."
    )
    reasoning: str = Field(
        description="A brief explanation of why this recommendation was made, referencing specific itinerary details."
    )
...
class SuggestionOption(BaseModel):
    action_type: str = Field(
        ...,
        description="Programmatic UI action token: 'PACE_UP', 'SWAP_DROP', or 'MANUAL_FORCE'.",
    )
    label: str = Field(
        ..., description="Friendly explanation of the choice for the button text."
    )
    target_swap_id: Optional[Any] = Field(
        None,
        description="If action_type is 'SWAP_DROP', the exact ID of a lower-priority attraction to remove.",
    )
...
class POIDiagnostic(BaseModel):
    ...
    reason: str = Field(
        ...,
        description="A short, clear explanation of why it couldn't fit into the current itinerary constraints.",
    )
    suggestions: List[SuggestionOption] = Field(
        ..., description="2-3 actionable ways the user can make this attraction fit."
    )


class ExplanationResponseSchema(BaseModel):
    explanations: List[POIDiagnostic] = Field(
        ..., description="Collection of diagnostics for the excluded attractions."
    )
