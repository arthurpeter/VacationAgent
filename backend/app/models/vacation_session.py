...
vacation_companions = Table(
    ...
    Column(
        "vacation_session_id",
        Integer,
        ForeignKey("vacation_sessions.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "companion_id",
        String,
        ForeignKey("travel_companions.id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
class VacationSession(Base):
    ...
    current_stage = Column(
        Enum(SessionStage), default=SessionStage.DISCOVERY, nullable=False
    )
    ...
    expires_at = Column(
        DateTime, default=get_expiry_date, onupdate=get_expiry_date, nullable=True
    )
